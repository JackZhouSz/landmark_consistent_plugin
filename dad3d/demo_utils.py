...
def draw_3d_landmarks(predictions: Dict[str, torch.Tensor], image: np.ndarray, subset: str = "191") -> np.ndarray:
    if subset != "191" and subset !="247" and subset != "445" and subset != "203":
...
def draw_pose(predictions: Dict[str, torch.Tensor], image: np.ndarray) -> np.ndarray:
    ...
    # print (image.shape, (int(tdx), int(tdy)), (int(x1), int(y1)), int(image.shape[0] * 0.005))
    cv2.arrowedLine(image, (int(tdx), int(tdy)), (int(x1), int(y1)), (0, 0, 255), max(int(image.shape[0] * 0.005),  1) )
    cv2.arrowedLine(image, (int(tdx), int(tdy)), (int(x2), int(y2)), (0, 255, 0), max(int(image.shape[0] * 0.005),  1))
    cv2.arrowedLine(image, (int(tdx), int(tdy)), (int(x3), int(y3)), (255, 0, 0), max(int(image.shape[0] * 0.005),  1))
