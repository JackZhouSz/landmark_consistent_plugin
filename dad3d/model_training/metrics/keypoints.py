...
def keypoints_nme(
    ...
    # norm_distance = 512.0 # for eg3d cropped images with res 512
...
def lm2d_68_eye_distance(lm2d: Tensor):
    '''
    lm2d: Tensor, (B, 68, 2)
    '''
    ...
    eye_distance = (eye_left - eye_right).pow(2).sum(1).sqrt()
    ...
    eye_mouth_distance = (mouth_avg - eye_avg).pow(2).sum(1).sqrt()
    
    return (eye_distance + eye_mouth_distance) / 2.0


def keypoints_nme_512(
    output_kp: Tensor,
    target_kp: Tensor,
    bbox: Tensor = None,
    reduce: str = "mean",
    ...
    # norm_distance = torch.sqrt(bbox[:, 2] * bbox[:, 3]) if bbox is not None else 2.0
    norm_distance = 512.0 # for eg3d cropped images with res 512
    # norm_distance = lm2d_68_eye_distance(target_kp)
    nme = torch.div(err, norm_distance)
    if reduce == "mean":
        nme = torch.mean(nme)
    return nme
...
class KeypointsNME512(Metric):
    ...
    def update(self, pred_keypoints: Tensor, gts: Dict[str, Tensor]) -> None:
        ...
        dim reflects 2D-3D mode.
        """
        self.nme += metrics_w_bbox_wrapper(function=keypoints_nme_512, outputs=pred_keypoints, gts=gts)
        self.total += 1
