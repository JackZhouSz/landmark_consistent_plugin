...
class FlameRegression(nn.Module):
    def __init__(self, model_config: Dict[str, Any], consts_config: Dict[str, Any], num_classes: int = 247):
        super().__init__()
    ...
    def forward(self, x):        
        ...
        if ENABLE_DEBUG:
            time_bifpn = time.time()
        heatmap = self.head(decoder_output)
        if ENABLE_DEBUG:
            time_flame= time.time()
        ...
        if ENABLE_DEBUG:
            ...
            print('===============FlameRegression================forward=======time_encoder:{}'.format(time_enc - time_st))
            print('===============FlameRegression================forward=======time_bifpn:{}'.format(time_bifpn - time_enc))
            print('===============FlameRegression================forward=======time_flame:{}'.format(time_flame - time_bifpn))
            print('===============FlameRegression================forward=======time_fusion:{}'.format(time_fusion - time_flame))
            print('===============FlameRegression================forward=======time_3dmm:{}'.format(time_3dmm - time_fusion))
            print('===============FlameRegression================forward=======time_lm:{}'.format(time_lm - time_3dmm))
