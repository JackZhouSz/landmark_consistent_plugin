...
from .ddfa import reconstruct_vertex, reconstruct_vertex_pytorch
...
def _predict_vertices_pytorch(param, roi_bbox, dense, transform=True):
    vertex = reconstruct_vertex_pytorch(param, dense=dense)
    sx, sy, ex, ey = roi_bbox
    scale_x = (ex - sx) / 120
    scale_y = (ey - sy) / 120
    vertex[:, 0, :] = vertex[:, 0, :] * scale_x + sx
...
def predict_68pts_pytorch(param, roi_box):
    return _predict_vertices_pytorch(param, roi_box, dense=False)
